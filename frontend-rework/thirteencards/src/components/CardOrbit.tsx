...
function CardOrbit() {
    ...
    useEffect(() => {
        ...
        function measure() {
            ...
            const height = Math.max(window.innerHeight - headerHeight, 0)

            // The inset has to clear half a card, but the card's size depends on
            // the path the inset defines. Two passes is enough to settle it.
        ...
        // Viewport changes drive the perimeter; the header drives the top edge.
        // body's height is content-driven and does not track the viewport, so a
        // ResizeObserver on it alone would miss a vertical resize.
        window.addEventListener('resize', remeasure)
        const header = document.querySelector('.site-header')
        const observer = header ? new ResizeObserver(remeasure) : null
